...
public class LocalDb extends SQLiteOpenHelper {
   ...
   // public static final String FR_STATUS = "FR_STATUS";
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        sqLiteDatabase.execSQL("create table " + FRIEND_EXIEST_TABLE +" (ID INTEGER PRIMARY KEY AUTOINCREMENT," +
                "FR_PHON TEXT,FR_NAME TEXT, FR_BLOCK TEXT,FR_EMRGCY)");
   ...
   public void deleteFriendReq(String id){
    ...
    public boolean insertExist(String F_PHON, String F_NAME)
    {
        ...
        contentValues.put("FR_PHON",F_PHON);
        contentValues.put("FR_NAME",F_NAME);
        contentValues.put("FR_EMRGCY","no");
        //  contentValues.put("FR_STATUS",F_STATUS);
    ...
    public boolean updateEmergency (String id,String tag)
    {
        SQLiteDatabase db=this.getWritableDatabase();

        try {
            if(tag.equalsIgnoreCase("make"))
            {

                db.execSQL("UPDATE " + FRIEND_EXIEST_TABLE + " SET FR_EMRGCY ='yes' WHERE ID ='" + id + "'");
                return true;
            }
            else
                {
                    db.execSQL("UPDATE " + FRIEND_EXIEST_TABLE + " SET FR_EMRGCY ='no' WHERE ID ='" + id + "'");
                    return true;
                }
        }
        catch (Exception ex)
